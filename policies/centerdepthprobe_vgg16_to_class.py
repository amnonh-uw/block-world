...
class DaggerPolicy(DaggerPolicyBase):
    ...
    def build_graph(self, dir_name):
        ...
        self.depthcam = tf.placeholder(tf.float32, shape=[None, self.width, self.height, 3], name='depthcam')
    ...
    def loss_feed_dict(self, batch):
        ...
        depthcam = batch['multichanneldepthcam']
        depthcam = depthcam.astype(np.uint16)
        depthcam = img_as_float(depthcam)
        depthcam = np.stack((depthcam, depthcam, depthcam), axis=-1)
